...
describe('test useModel', () => {
	...
	describe('reducers dispatch', () => {
		...
		test('reducer with payload and meta', () => {
			function App() {
				const [{ step }, { addOneStepWithMeta }] = useModel(stepModel)
				return (
					<div>
						<div id="step">step:{step}</div>
						<div id="button" onClick={() => addOneStepWithMeta(3, 7)}>
							addValue
						</div>
					</div>
				)
			...
			expect(node.querySelector('#step')?.innerHTML).toEqual('step:11')
		})
	})
	...
	describe('effects dispatch without dependency models', () => {
		...
		test('effect with payload and meta', () => {
			function App() {
				...
				return (
					<div>
						...
						<div
							id="button"
							onClick={() => addValueByEffectWithPaloadAndMeta(4, 5)}
						>
							addValue
						</div>
					</div>
			...
			expect(node.querySelector('#addValue')?.innerHTML).toEqual('value:10')
		})
