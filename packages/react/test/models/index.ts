  ...
  state: {
	...
	reducers: {
      ...
      return {
		...
		addOneStepWithMeta(state, payload: number, meta: number) {
      return {
        ...state,
        step: state.step + payload + meta
      };
    },
...
export const countModel = defineModel({
  ...
  state: {
	...
	effects: {
		...
		addValueByEffectWithPaloadAndMeta(payload: number, _state, _depends, meta: number) {
			this.addValueByParam(payload + meta)
		},
